...
drone.on("open", error => {
    ...
    room = drone.subscribe(roomName); //creation of rooom via roomNAme
    room.on("open", error => {
        if (error) {
            onError(error);
        }
...
function startWebRTC(isOfferer) {
    ...
    if (isOfferer) {
        pc.onnegotiationneeded = () => {
            pc.createOffer() //reteuns promise
    ...
    room.on("data", (message, client) => {
        ...
        if (message.sdp) { // session description protocol (Mozilla site)
            ...
            pc.setRemoteDescription(
                ...
                () => {
                    ...
                    if (pc.remoteDescription.type === "offer") {
                        pc.createAnswer()
                            .then(localDescCreated) //create answer if remoteDescription is of ypw "offer"
                            .catch(onError);
                    }
                },
                onError
